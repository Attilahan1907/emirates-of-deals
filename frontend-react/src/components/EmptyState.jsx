...
export function EmptyState({ variant = 'welcome', onSuggestionClick }) {
  if (variant === 'no-results') {
    return (
      <div className="flex flex-col items-center justify-center py-20 text-center">
        ...
        <h2 className="text-lg font-medium text-foreground/60 mb-1">Keine Ergebnisse</h2>
        <p className="text-sm text-foreground/30 max-w-md">
  ...
  return (
    <div className="flex flex-col items-center justify-center py-20 text-center">
      ...
      <p className="text-sm text-foreground/30 max-w-md mb-8">
      ...
      <div className="flex flex-wrap justify-center gap-2">
        {SUGGESTIONS.map((s) => (
          <button
            ...
            className="glass rounded-full px-4 py-2 text-xs text-foreground/50 hover:text-neon-cyan hover:border-neon-cyan/30 transition-all cursor-pointer"
          >
            {s}
          </button>
        ))}
